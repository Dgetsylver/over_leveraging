...
async function main() {
  ...
  // Use method names from the SDK
  const methods = Object.getOwnPropertyNames(Object.getPrototypeOf(val)).filter(m => typeof (val as any)[m] === 'function' && m !== 'constructor');
  console.log("Available methods on val:", methods.join(", "));

  // Try different accessor patterns
  for (const method of methods) {
    try {
      const result = (val as any)[method]();
      if (result && typeof result === 'object') {
        const innerMethods = Object.getOwnPropertyNames(Object.getPrototypeOf(result))
          .filter(m => typeof result[m] === 'function' && m !== 'constructor');
        console.log(`val.${method}() -> object with methods: ${innerMethods.slice(0, 10).join(", ")}`);
        
        // Try to get executable/storage
        if (innerMethods.includes('executable')) {
          const exec = result.executable();
          console.log(`  executable type: ${exec.switch().name}`);
          if (exec.switch().name === 'contractExecutableWasm') {
            try { console.log(`  WASM hash: ${exec.wasmHash().toString('hex')}`); } catch {}
          }
        }
        if (innerMethods.includes('storage')) {
          const st = result.storage();
          if (st) {
            console.log(`  storage entries: ${st.length}`);
